...
function imghaste_section_pwa_cb( $args ) {
	?><p id="<?php echo esc_attr( $args['id'] ); ?>"><?php esc_html_e( 'Settings to set up your PWA. Enable to use!', 'imghaste' ); ?></p><?php
}
...
function imghaste_field_pwa_appname_cb( $args ) {
	...
	if(isset($_POST['imghaste_field_pwa_appname'])){
		$field_value = $_POST('imghaste_field_pwa_appname');
	} else if(isset($options['imghaste_field_pwa_appname'])){
...
function imghaste_field_pwa_short_appname_cb( $args ) {
	...
	if(isset($_POST['imghaste_field_pwa_short_appname'])){
		$field_value =$_POST('imghaste_field_pwa_short_appname');
	}else if(isset($options['imghaste_field_pwa_short_appname'])){
...
function imghaste_field_pwa_description_cb( $args ) {
	...
	if(isset($_POST['imghaste_field_pwa_description'])){
		$field_value = $_POST('imghaste_field_pwa_description');
	}else if(isset($options['imghaste_field_pwa_description'])){
...
function imghaste_field_pwa_background_color_cb( $args ) {
	...
	if(isset($_POST['imghaste_field_pwa_background_color'])){
		$field_value = $_POST('imghaste_field_pwa_background_color');
	}else if(isset($options['imghaste_field_pwa_background_color'])){
...
function imghaste_field_pwa_theme_color_cb( $args ) {
	...
	if(isset($_POST['imghaste_field_pwa_theme_color'])){
		$field_value =$_POST('imghaste_field_pwa_theme_color');
	}else if(isset($options['imghaste_field_pwa_theme_color'])){
...
function imghaste_field_pwa_start_url_cb() {
	...
	if(isset($_POST['imghaste_field_pwa_start_url'])){
		$field_value = esc_url($_POST('imghaste_field_pwa_start_url'));
	}else if(isset($options['imghaste_field_pwa_start_url'])){
		$field_value = $options['imghaste_field_pwa_start_url'];
	}else{ 
		$field_value = '';
	} 
	...
	<fieldset>
		...
		<p class="description">
			<?php
			$show_url = Imghaste_PWA::imghaste_get_start_url() ;
			if(empty($show_url)){
				$show_url = 'not set';
			}
			printf( __( 'Specify the page to load when the application is launched from a device. Current start page is <code>%s</code>', 'imghaste' ), $show_url ); ?>
		</p>
		
		<?php if ( Imghaste_PWA::imghaste_is_amp() ) { ?>
